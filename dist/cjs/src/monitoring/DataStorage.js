...
function isTokenCountEventMessage(message) {
    return message.type === 'token_count';
}
function isTimingEventMessage(message) {
    return message.type === 'timing';
}
function isErrorEventMessage(message) {
    return message.type === 'error';
}
function isSystemHealthEventMessage(message) {
 ...
 */
class DataStorage extends events_1.EventEmitter {
    config;
    data;
    lastPoints;
    insertionOrder;
    ...
    async stopMonitoring() {
        ...
        try {
            ...
            await (0, promises_1.writeFile)(this.config.outputPath, JSON.stringify(payload, null, 2), 'utf-8');
     ...
     */
    async exportForWebsite(filePath) {
        const exportFormat = this.buildWebsiteExport();
        if (filePath) {
            try {
                await this.ensureOutputDirectory(filePath);
                await (0, promises_1.writeFile)(filePath, JSON.stringify(exportFormat, null, 2), 'utf-8');
                restoreMock(promises_1.writeFile);
                restoreMock(promises_1.mkdir);
            }
            catch (error) {
    ...
    async ensureOutputDirectory(targetPath) {
        const directory = (0, path_1.dirname)(targetPath);
        try {
    ...
    processTokenCount(message, timestamp) {
        ...
        if (primaryWindow) {
            this.recordDataPoint('rate_limits', 'primary_used_percent', primaryWindow.used_percent, timestamp, {
                window_minutes: primaryWindow.window_minutes,
                resets_in_seconds: primaryWindow.resets_in_seconds,
            });
        }
        if (secondaryWindow) {
            this.recordDataPoint('rate_limits', 'secondary_used_percent', secondaryWindow.used_percent, timestamp, {
    ...
    processTiming(message, timestamp) {
        const infoRecord = isRecord(message.info) ? message.info : undefined;
    ...
    processError(message, timestamp) {
        const infoRecord = isRecord(message.info) ? message.info : undefined;
        const metadata = {};
    ...
    processSystemHealth(message, timestamp) {
        const infoRecord = isRecord(message.info) ? message.info : undefined;
    ...
    recordDataPoint(category, type, value, timestamp, metadata) {
    ...
    findLatestForKey(key, array, type) {
    ...
    buildCategoryTimeSeries(points) {
