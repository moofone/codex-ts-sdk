...
const bindings_1 = require("./internal/bindings");
const converters_1 = require("./internal/converters");
const envFallback_1 = require("./internal/envFallback");
/**
...
class CloudTasksClient {
    options;
    native = (0, bindings_1.getCloudBindings)();
    nativeConfig;
    resolvedConfig;
    closed = false;
    constructor(options) {
    ...
    normalizeConfig(options) {
        ...
        const nativeConfig = (0, bindings_1.toNativeConfig)({
            ...options,
            baseUrl: resolvedBaseUrl,
            mock: resolvedMock,
        });
    ...
    async listTasks(options) {
        try {
            ...
            const mapped = list.map(converters_1.toTaskSummary);
            ...
            throw (0, errors_1.toCloudTasksError)(err);
        }
    }
    /**
    ...
    async listEnvironments() {
        try {
            ...
            const fallback = await (0, envFallback_1.listEnvironmentsFallback)(this.resolvedConfig);
            return fallback;
        }
        catch (err) {
            throw (0, errors_1.toCloudTasksError)(err);
     ...
     */
    async resolveEnvironmentId(value) {
        ...
        if (!match)
            throw new Error(`Environment not found by label: ${value}`);
        return match.id;
    }
    /**
     ...
     * ```
     */
    async getTaskText(taskId) {
        try {
            const n = await this.native.getText(this.nativeConfig, taskId);
            return (0, converters_1.toTaskText)(n);
        }
        catch (err) {
            throw (0, errors_1.toCloudTasksError)(err);
        }
     ...
     * ```
     */
    async applyTaskPreflight(taskId, options) {
        try {
            const { diffOverride, preflight } = (0, bindings_1.toNativeApplyParams)(taskId, { ...options, dryRun: true });
            const outcome = await this.native.apply(this.nativeConfig, taskId, diffOverride, preflight);
            return (0, converters_1.toApplyOutcome)(outcome);
        }
        catch (err) {
            throw (0, errors_1.toCloudTasksError)(err);
        }
     ...
     * ```
     */
    async applyTask(taskId, options) {
        try {
            const { diffOverride, preflight } = (0, bindings_1.toNativeApplyParams)(taskId, options);
            const outcome = await this.native.apply(this.nativeConfig, taskId, diffOverride, preflight);
            return (0, converters_1.toApplyOutcome)(outcome);
        }
        catch (err) {
     ...
     * ```
     */
    async listSiblingAttempts(taskId, turnId) {
        try {
            const list = await this.native.listAttempts(this.nativeConfig, taskId, turnId);
            return list.map(converters_1.toTurnAttempt);
        }
        catch (err) {
            throw (0, errors_1.toCloudTasksError)(err);
        }
    ...
    async tryListEnvironmentsNative() {
        ...
        try {
            ...
            return rows.map(r => ({
                id: r.id,
                label: r.label,
                isPinned: r.isPinned ?? r.is_pinned,
                repoHints: r.repoHints ?? r.repo_hints,
            }));
        }
        catch (err) {
