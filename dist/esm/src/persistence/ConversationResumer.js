...
import { ValidationError as ValidationErrorClass, ResumptionError, ResumptionTimeoutError, SIDE_EFFECT_EVENT_TYPES, } from '../types/resumption';
/**
...
export class ConversationResumer extends EventEmitter {
        ...
        try {
            ...
            if (opts.validateData) {
                ...
                if (!validationResult.isValid) {
                    throw new ValidationErrorClass('Rollout data validation failed', validationResult.errors);
                }
            }
            else {
                validationResult = {
    ...
    isSideEffectEvent(event) {
        return SIDE_EFFECT_EVENT_TYPES.includes(event.msg.type);
    }
    /**
        ...
        const replayPromise = (async () => {
            for (let i = 0; i < events.length; i++) {
                ...
                try {
                    ...
                    failureCount++;
                    const resumptionError = new ResumptionError(`Failed to replay event ${i}: ${error instanceof Error ? error.message : String(error)}`, 'REPLAY_ERROR', i, true);
                    if (this.currentState) {
...
//# sourceMappingURL=ConversationResumer.js.map