...
import { RolloutFileError, RolloutSerializationError, } from '../types/rollout';
...
export class RolloutRecorder extends EventEmitter {
    ...
    async startRecording(client, customSessionMetadata) {
        ...
        try {
            ...
            throw new RolloutSerializationError(`Failed to start recording: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    async stopRecording() {
        ...
        try {
            ...
            throw new RolloutFileError(`Failed to stop recording: ${error instanceof Error ? error.message : String(error)}`, this.outputPath || 'unknown', 'finalize');
        }
        finally {
            this.cleanup();
        }
    ...
    async initializeSessionMetadata(customMetadata) {
        try {
            ...
            throw new RolloutSerializationError(`Failed to initialize session metadata: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    handleEvent = (event) => {
        ...
        try {
            ...
            this.emit('recordingError', {
                error: new RolloutSerializationError(`Failed to record event: ${error instanceof Error ? error.message : String(error)}`, event),
            });
        }
    };
    /**
    ...
    writeSessionHeader() {
        ...
        try {
            ...
            throw new RolloutFileError(`Failed to write session header: ${error instanceof Error ? error.message : String(error)}`, this.outputPath, 'write');
        }
    }
    /**
    ...
    writeEventEntry(eventEntry) {
        ...
        try {
            ...
            throw new RolloutFileError(`Failed to write event entry: ${error instanceof Error ? error.message : String(error)}`, this.outputPath, 'append');
        }
    }
    /**
     * Cleanup resources
...
//# sourceMappingURL=RolloutRecorder.js.map